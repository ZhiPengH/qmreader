...
db.exec(`
  ...
  CREATE TABLE IF NOT EXISTS plaza_tag_usage (
    user_id TEXT NOT NULL,
    day TEXT NOT NULL,
  ...
  CREATE TABLE IF NOT EXISTS plaza_interests (
    user_id TEXT NOT NULL,
    name TEXT NOT NULL,
...
ensureColumn('entry_topics', 'card_ratio', 'card_ratio TEXT');
...
// All devices use this dedicated identity. Historical users and sessions remain
// stored for compatibility, but are never used to resolve the current reader.
function getPersonalUser() {
  const id = 'zonglan-personal';
  const email = 'personal@zonglan.invalid';
  const t = now();
  db.prepare(`
  ...
  `).run(id, email, t, t);
  // Even an old or manually altered row must never grant this identity admin rights.
  db.prepare("UPDATE users SET role = 'user' WHERE id = ? AND role <> 'user'").run(id);
...
// Personal reading records outlive the rolling feed cache and list limit.
function getUserRetainedEntries(userId) {
  return db.prepare(`
    SELECT e.*, t.title_zh, t.summary_zh, t.title_hash AS translation_title_hash
    FROM entries e
    JOIN user_entry_states s ON s.entry_id = e.id AND s.user_id = ?
    LEFT JOIN entry_translations t ON t.entry_id = e.id
    WHERE COALESCE(e.deleted_at, 0) = 0
      AND (s.read_at IS NOT NULL OR s.starred_at IS NOT NULL OR s.viewed_at IS NOT NULL)
  `).all(userId).map(normalizeEntry);
}

function plazaError(message, statusCode = 400) {
  const error = new Error(message);
  error.statusCode = statusCode;
  return error;
}

function normalizePlazaTag(tag) {
  if (!tag || typeof tag.name !== 'string' || /[<>\p{Cc}\p{Cf}]/u.test(tag.name)) {
    throw plazaError('invalid tag name');
  }
  const name = tag.name.trim();
  const kind = tag.kind === undefined ? 'topic' : tag.kind;
  if (!name || [...name].length > 24 || !['topic', 'format'].includes(kind)) {
    throw plazaError('invalid tag name or kind');
  }
  return { name, kind };
}

function normalizePlazaTags(tags) {
  if (!Array.isArray(tags) || tags.length > 8) throw plazaError('tags must contain at most 8 labels');
  const unique = new Map();
  for (const value of tags) {
    const tag = normalizePlazaTag(value);
    unique.set(JSON.stringify([tag.name, tag.kind]), tag);
  }
  return [...unique.values()];
}

function requirePlazaEntry(entryId) {
  if (typeof entryId !== 'string' || !db.prepare('SELECT id FROM entries WHERE id = ? AND COALESCE(deleted_at, 0) = 0').get(entryId)) {
    throw plazaError('entry not found', 404);
  }
}

function getEntryTopics(entryId) {
  requirePlazaEntry(entryId);
  const row = db.prepare('SELECT * FROM entry_topics WHERE entry_id = ?').get(entryId);
...
function saveEntryTopics(entryId, {
  tags, status = 'ready', origin = 'ai', inputHash = '', model = '', provider = '', error = '',
}) {
  requirePlazaEntry(entryId);
  const cleanTags = normalizePlazaTags(tags);
  if (!['pending', 'ready', 'failed'].includes(status) || !['ai', 'manual'].includes(origin)) {
    throw plazaError('invalid tag status or origin');
  }
  if (status === 'ready' && origin === 'ai' && !cleanTags.length) {
    throw plazaError('AI ready tags cannot be empty');
  }
  db.prepare(`
    INSERT INTO entry_topics (entry_id, tags_json, status, origin, input_hash, model, provider, error, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    ON CONFLICT(entry_id) DO UPDATE SET
      tags_json = excluded.tags_json, status = excluded.status, origin = excluded.origin,
      input_hash = excluded.input_hash, model = excluded.model, provider = excluded.provider,
      error = excluded.error, updated_at = excluded.updated_at
    WHERE entry_topics.origin <> 'manual' OR excluded.origin = 'manual'
  `).run(entryId, JSON.stringify(cleanTags), status, origin, inputHash, model, provider, error, now());
  return getEntryTopics(entryId);
}

function getPlazaCardRatio(row) {
  if (row.card_ratio) return row.card_ratio;
  const ratios = row.image ? ['3/4', '9/16', '1/2']
    : plainTextLength(row.title) >= 60 || plainTextLength(row.summary) >= 160 ? ['3/4', '4/3']
      : ['4/3', '16/9'];
  const ratio = ratios[Number.parseInt(hashText(row.id).slice(0, 8), 16) % ratios.length];
  // Keep the first assignment even if another process filled it after our read.
  return db.prepare(`
    INSERT INTO entry_topics (entry_id, card_ratio) VALUES (?, ?)
    ON CONFLICT(entry_id) DO UPDATE SET card_ratio = COALESCE(entry_topics.card_ratio, excluded.card_ratio)
    RETURNING card_ratio
  `).get(row.id, ratio).card_ratio;
}

// Plaza reads persisted metadata, not the rolling fetcher cache or article bodies.
function getPlazaEntries({ userId = '', ids = null, maxRowId = null } = {}) {
  if (maxRowId !== null) requirePlazaInteger(maxRowId, 'maxRowId');
  if (ids !== null && (!Array.isArray(ids) || ids.some(id => typeof id !== 'string'))) {
    throw plazaError('ids must be an array of strings');
  }
  if (ids && !ids.length) return [];
  const rows = db.prepare(`
    SELECT e.id, e.source_id, e.title, e.link, e.author, e.published, e.published_ts,
           e.summary, e.image, e.audio_json, e.content_hash, e.original_fetched_at,
           e.original_fetch_attempted_at, e.original_fetch_error,
           e.deleted_at, e.deleted_by, e.deleted_reason, e.created_at,
           t.title_zh, t.summary_zh, t.title_hash AS translation_title_hash,
           s.read_at, r.reaction, p.tags_json, p.status AS tag_status, p.origin AS tag_origin,
           p.input_hash AS tag_input_hash, p.error AS tag_error, p.updated_at AS tag_updated_at, p.card_ratio
    FROM entries e
    LEFT JOIN entry_translations t ON t.entry_id = e.id
    LEFT JOIN user_entry_states s ON s.entry_id = e.id AND s.user_id = ?
    LEFT JOIN entry_reactions r ON r.entry_id = e.id AND r.user_id = ?
    LEFT JOIN entry_topics p ON p.entry_id = e.id
    WHERE COALESCE(e.deleted_at, 0) = 0
      AND (? IS NULL OR e.rowid <= ?)
      ${ids === null ? '' : 'AND e.id IN (SELECT value FROM json_each(?))'}
    ORDER BY e.published_ts DESC, e.rowid DESC
  `).all(userId, userId, maxRowId, maxRowId, ...(ids === null ? [] : [JSON.stringify(ids)])).map(row => {
    const { content, ...metadata } = normalizeEntry(row);
    return {
      ...metadata,
      createdAt: row.created_at,
      cardRatio: getPlazaCardRatio(row),
      read: Boolean(row.read_at),
      reactionByMe: row.reaction || '',
      tags: safeJsonParse(row.tags_json, []),
      tagStatus: row.tag_status || 'pending',
      tagOrigin: row.tag_origin || '',
      tagInputHash: row.tag_input_hash || '',
      tagError: row.tag_error || '',
      tagUpdatedAt: row.tag_updated_at || null,
    };
  });
...
function requirePlazaInteger(value, name) {
  if (!Number.isSafeInteger(value) || value < 0) throw plazaError(`${name} must be a nonnegative safe integer`);
}

function getPlazaLibraryStatus(afterRowId = 0) {
  requirePlazaInteger(afterRowId, 'afterRowId');
  const row = db.prepare(`
    SELECT MAX(rowid) AS revision,
           SUM(CASE WHEN COALESCE(deleted_at, 0) = 0 THEN 1 ELSE 0 END) AS total,
           SUM(CASE WHEN COALESCE(deleted_at, 0) = 0 AND rowid > ? THEN 1 ELSE 0 END) AS new_count
    FROM entries
  `).get(afterRowId);
  return { revision: row.revision || 0, total: row.total || 0, newCount: row.new_count || 0 };
}

const plazaDayFormatter = new Intl.DateTimeFormat('en-CA', {
  timeZone: 'Asia/Shanghai', year: 'numeric', month: '2-digit', day: '2-digit',
});

function getPlazaTagUsage(userId, limit, at = Date.now()) {
  requirePlazaUser(userId);
  requirePlazaInteger(limit, 'limit');
  if (typeof at !== 'number' || !Number.isFinite(at) || Number.isNaN(new Date(at).getTime())) {
    throw plazaError('invalid allowance timestamp');
  }
  const day = plazaDayFormatter.format(at);
  const row = db.prepare('SELECT used FROM plaza_tag_usage WHERE user_id = ? AND day = ?').get(userId, day);
  const used = row ? row.used : 0;
  return { day, limit, used, remaining: Math.max(0, limit - used) };
}

function claimPlazaTagAllowance(userId, count, limit, at = Date.now()) {
  requirePlazaInteger(count, 'count');
  if (count === 0) return { ...getPlazaTagUsage(userId, limit, at), granted: 0 };
  db.exec('BEGIN IMMEDIATE');
  try {
    const usage = getPlazaTagUsage(userId, limit, at);
    const granted = Math.min(count, usage.remaining);
    if (granted) {
      db.prepare(`
        INSERT INTO plaza_tag_usage (user_id, day, used) VALUES (?, ?, ?)
        ON CONFLICT(user_id, day) DO UPDATE SET used = excluded.used
      `).run(userId, usage.day, usage.used + granted);
    }
    db.exec('COMMIT');
    return { ...usage, used: usage.used + granted, remaining: usage.remaining - granted, granted };
  } catch (error) {
    db.exec('ROLLBACK');
    throw error;
  }
...
function requirePlazaUser(userId) {
  if (typeof userId !== 'string' || !userId || !db.prepare('SELECT id FROM users WHERE id = ?').get(userId)) {
    throw plazaError('invalid userId');
  }
}

function setPlazaInterest(userId, tag, interested = true) {
  requirePlazaUser(userId);
  if (typeof interested !== 'boolean') throw plazaError('interested must be boolean');
  const { name, kind } = normalizePlazaTag(tag);
  if (interested) {
    db.prepare('INSERT OR IGNORE INTO plaza_interests (user_id, name, kind, created_at) VALUES (?, ?, ?, ?)')
      .run(userId, name, kind, now());
  } else {
    db.prepare('DELETE FROM plaza_interests WHERE user_id = ? AND name = ? AND kind = ?').run(userId, name, kind);
  }
  return getPlazaPreferences(userId);
}

function getPlazaPreferences(userId) {
  const interests = db.prepare('SELECT name, kind FROM plaza_interests WHERE user_id = ? ORDER BY name, kind')
    .all(userId).map(row => ({ name: row.name, kind: row.kind }));
  const ignoredIds = db.prepare(`
    SELECT entry_id FROM entry_reactions
    WHERE user_id = ? AND reaction = 'dislike'
    ORDER BY updated_at DESC, entry_id
  `).all(userId).map(row => row.entry_id);
  const knownTags = new Map();
  for (const row of db.prepare(`
    SELECT p.tags_json FROM entry_topics p
    JOIN entries e ON e.id = p.entry_id
    WHERE p.status = 'ready' AND COALESCE(e.deleted_at, 0) = 0
    ORDER BY e.rowid
  `).all()) {
    for (const tag of safeJsonParse(row.tags_json, [])) {
      knownTags.set(JSON.stringify([tag.name, tag.kind]), tag);
    }
  }
  for (const tag of interests) knownTags.set(JSON.stringify([tag.name, tag.kind]), tag);
  return {
    interests,
    ignored: getPlazaEntries({ userId, ids: ignoredIds }),
    knownTags: [...knownTags.values()],
  };
}
...
function disableUserForModeration(userId, { adminUserId = '', reason = '' } = {}) {
  ...
  return { user, deletedSubmissionCount };
}
...
function getSummary(entryId) {
  const row = db.prepare('SELECT * FROM entry_summaries WHERE entry_id = ?').get(String(entryId || ''));
  return row ? publicSummary(row) : null;
}

function saveSummary(entryId, summary) {
  const t = now();
  db.prepare(`
    INSERT INTO entry_summaries (
      entry_id, user_id, body, model, provider, created_by, content_hash, created_at, updated_at
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    ON CONFLICT(entry_id) DO UPDATE SET
      user_id = excluded.user_id,
      body = excluded.body,
  ...
  `).run(
    ...
    publicAuthor(summary.createdBy || 'system'),
    summary.contentHash || '',
    t,
    t,
  );
...
function rowToPrefetchEntry(row) {
  return {
    id: row.id,
    sourceId: row.source_id,
    title: row.title || '',
    ...
    originalFetchedAt: row.original_fetched_at || null,
    originalFetchAttemptedAt: row.original_fetch_attempted_at || null,
    originalFetchError: row.original_fetch_error || null,
  };
}

function getRecentEntriesForPrefetch(limit = 2000) {
  const rows = db.prepare(`
    SELECT id, source_id, title, link, published_ts, summary, content,
           original_fetched_at, original_fetch_attempted_at, original_fetch_error
    FROM entries
    WHERE link LIKE 'http%'
    ORDER BY published_ts DESC
    LIMIT ?
  `).all(Math.max(1, Number(limit) || 2000));
  return rows.map(rowToPrefetchEntry).filter(entry => !isEntryDeleted(entry.id));
}

function offlinePrefetchBytes() {
  const row = db.prepare("SELECT COALESCE(SUM(LENGTH(content)), 0) AS bytes FROM entries WHERE original_fetched_at IS NOT NULL AND content <> ''").get();
  return Number(row && row.bytes) || 0;
}

function enforceOfflineCacheBudget(maxBytes) {
  const budget = Math.max(0, Number(maxBytes) || 0);
  const total = offlinePrefetchBytes();
  if (!budget || total <= budget) return { bytes: total, evicted: 0 };
  const rows = db.prepare(`
    SELECT id, LENGTH(content) AS size FROM entries
    WHERE original_fetched_at IS NOT NULL AND content <> ''
    ORDER BY original_fetched_at ASC, published_ts ASC
  `).all();
  const evictIds = [];
  let running = total;
  for (const row of rows) {
  ...
  if (evictIds.length) {
    db.prepare(`UPDATE entries SET content = '', original_fetched_at = NULL, updated_at = ? WHERE id IN (${evictIds.map(() => '?').join(',')})`).run(now(), ...evictIds);
  }
  return { bytes: offlinePrefetchBytes(), evicted: evictIds.length };
}

function purgeDeletedSources() {
  const overrides = getSourceOverrides().filter(item => item && item.deleted);
  const t = now();
  let purged = 0;
  for (const item of overrides) {
    if (String(item.id || '').startsWith('rss-')) {
      db.prepare('DELETE FROM source_overrides WHERE id = ?').run(item.id);
    } else {
      db.prepare('UPDATE source_overrides SET data_json = ?, updated_at = ? WHERE id = ?')
        .run(JSON.stringify({ ...item, purged: true }), t, item.id);
    }
    purged += 1;
  }
  return { purged };
...
// Subscription overrides are read on demand so server and worker processes agree.
db.exec(`CREATE TABLE IF NOT EXISTS source_overrides (
  id TEXT PRIMARY KEY,
  data_json TEXT NOT NULL,
  updated_at INTEGER NOT NULL
)`);
function getSourceOverrides() {
  return db.prepare('SELECT id, data_json FROM source_overrides').all()
    .map(row => ({ id: row.id, ...JSON.parse(row.data_json) }));
}
function saveSourceOverride(id, value) {
  db.prepare(`INSERT INTO source_overrides (id, data_json, updated_at) VALUES (?, ?, ?)
    ON CONFLICT(id) DO UPDATE SET data_json = excluded.data_json, updated_at = excluded.updated_at`)
    .run(id, JSON.stringify(value), Date.now());
}
function migrateSourceEnabled(id, enabled) {
  db.prepare('INSERT OR IGNORE INTO source_overrides (id, data_json, updated_at) VALUES (?, ?, ?)')
    .run(id, JSON.stringify({ enabled }), Date.now());
}
...
module.exports = {
  ...
  getPlazaPreferences,
  setPlazaInterest,
  getPlazaLibraryStatus,
  getPlazaTagUsage,
  claimPlazaTagAllowance,
  getUserRetainedEntries,
  getSourceOverrides,
  saveSourceOverride,
  migrateSourceEnabled,
  getPersonalUser,
  addAnnotation,
  addAnnotationReply,
  addChatMessage,
  ...
  getSummary,
  saveSummary,
  getRecentEntriesForPrefetch,
  offlinePrefetchBytes,
