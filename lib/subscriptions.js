  ...
  if ('feeds' in input || !existing) {
    ...
    if (!existing || JSON.stringify(input.feeds) !== JSON.stringify(existing.feeds)) {
      ...
      for (const url of out.feeds) {
        // Twitter user feeds are validated through the trusted internal channel.
        const candidates = (url.startsWith('{rsshub}') && routing.isTwitterFeed(url))
...
const defaultCheck = async (url, options = {}) => {
  ...
  const { isInternalRsshubTarget } = routing;
  if (isInternalRsshubTarget(url)) {
    const parsed = new URL(url);
    if (!/^\/twitter\/user\/[A-Za-z0-9_]{1,15}$/.test(parsed.pathname)) throw new Error('仅支持内部 RSSHub 的 twitter/user 路由');
