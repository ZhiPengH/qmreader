...
function expandRsshub(url) {
  // Twitter user feeds resolve through the private internal channel when
  // configured; everything else keeps the public multi-instance behaviour.
  return routing.expandFeedCandidates(String(url || ''), routing.internalOrigin() && routing.internalOrigin().origin);
}
  ...
  if ('feeds' in input || !existing) {
    ...
    if (!existing || JSON.stringify(input.feeds) !== JSON.stringify(existing.feeds)) {
      ...
      for (const url of out.feeds) {
        // Internal-channel feeds (twitter user, whitelisted exact routes) are
        // validated through the trusted internal channel.
        const candidates = (url.startsWith('{rsshub}') && routing.isInternalRouteFeed(url))
          ? routing.expandFeedCandidates(url, routing.internalOrigin()?.origin)
          : expandRsshub(url);
        let lastError = null;
        let reachable = false;
...
const defaultCheck = async (url, options = {}) => {
  // Internal RSSHub targets are validated through the restricted transport;
  // they are trusted by configuration and must not pass the public DNS guard.
  const { isInternalRsshubTarget, internalRouteFromUrl } = routing;
  if (isInternalRsshubTarget(url)) {
    const parsed = new URL(url);
    if (!/^\/twitter\/user\/[A-Za-z0-9_]{1,15}$/.test(parsed.pathname) && !internalRouteFromUrl(url)) {
      throw new Error('仅支持内部 RSSHub 的白名单路由');
    }
    await require('./fetcher').fetchInternalText(url, { timeout: 5000 });
    return url;
  }
  return require('./fetcher').assertPublicHttpUrl(url, options);
};
async function createSource(input, { checkUrl = defaultCheck } = {}) {
  ...
  // Twitter timelines carry many short posts; keep the wider entry window.
  if (Array.isArray(source.feeds) && source.feeds.some(feed => routing.isTwitterFeed(feed))) source.limit = 30;
...
async function updateSource(id, input, { checkUrl = defaultCheck } = {}) {
  ...
  if (fields.feeds) {
    next.limit = fields.feeds.some(feed => routing.isTwitterFeed(feed)) ? 30 : (existing.limit || 20);
  }
  store.saveSourceOverride(id, next);
